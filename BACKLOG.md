...
3. **`block/properties` is unwritable over JSON.** Found in LF-38, and the same root cause as (2) one level deeper. muuntaja keywordizes JSON object keys, so a property map arrives with *keyword* keys, but `bfs/enhance-props` (`src/cljc/athens/common_events/bfs.cljc:41-45`) uses that key as a **page title string**: it becomes the new block's position, `{:relation {:page/title <key>}}`. A keyword there fails malli validation — and then malli's own error formatter throws while trying to describe the failure, because it treats the offending value as a path index (`malli.error/-ensure` → `Numbers.gt`). The request dies with `500 class clojure.lang.Keyword cannot be cast to class java.lang.Number`, which names neither the field nor the real problem. Fix alongside (2): coerce property keys to strings at the API boundary. The same EDN request succeeds unchanged, which is how the MCP bridge works around it today.

Also consider: there is no exception middleware, so any rejected request surfaces as a 500 carrying the thrown message. That text is genuinely the most useful part of the response, so if you add middleware, preserve it. Note that defect (3) is the case where that convention fails — the 500 carries a cast error from inside the error formatter, not the real reason.

*Acceptance:* `"data":[]` returns a clean 400-class error with a readable message; a decision is made and documented on `relation`; a JSON write carrying `block/properties` either succeeds or fails with a message naming the property key.
